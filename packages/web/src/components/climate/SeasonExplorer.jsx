// src/components/climate/SeasonExplorer.jsx
/**
 * SeasonExplorer (Grow web)
 *
 * Ported from Regional Insights, trimmed to drop the "Compare Zones" mode since
 * the Climate History page already organises by the company's own zones.
 *
 * Modes: Overview / Monthly View / Compare Seasons.
...
import {
  TrendingUp, TrendingDown, Minus, Calendar, Droplets, Thermometer, Sun,
  BarChart3, LineChart, ChevronLeft, ChevronRight,
} from 'lucide-react';
import {
  getZoneSeasons,
  getZoneHistory,
...
const SeasonExplorer = ({ zone }) => {
  ...
  // Controls
  const [viewMode, setViewMode] = useState('overview'); // overview, monthly, season-compare
  ...
  const getLineChartOptions = () => ({
    ...
    scales: {
      ...
      x: {
        title: {
          display: viewMode === 'monthly' || viewMode === 'season-compare',
    ...
    <div className="season-explorer">
      ...
      <div className="chart-controls">
        <div className="chart-metric-selector">
          <button className={`chart-type-btn ${chartMetric === 'gdd' ? 'active' : ''}`} onClick={() => setChartMetric('gdd')}>GDD</button>
          <button className={`chart-type-btn ${chartMetric === 'rain' ? 'active' : ''}`} onClick={() => setChartMetric('rain')}>Rainfall</button>
          <button className={`chart-type-btn ${chartMetric === 'tmean' ? 'active' : ''}`} onClick={() => setChartMetric('tmean')}>Avg Temp</button>
          <button className={`chart-type-btn ${chartMetric === 'tmax' ? 'active' : ''}`} onClick={() => setChartMetric('tmax')}>Max Temp</button>
        </div>
        ...
        <div className="seasons-header">
          ...
          {totalPages > 1 && (
            <div className="seasons-pagination">
              ...
              <span className="page-info">{seasonPage + 1} / {totalPages}</span>
              <button
                className="page-btn"
