...
const STATUS_COLORS = {
  ...
  pending: { bg: 'var(--color-warning-bg)', text: 'var(--color-warning-text)', label: 'Pending' },
  suspended: { bg: 'var(--color-danger-bg)', text: 'var(--color-danger-text)', label: 'Suspended' },
  terminated: { bg: 'var(--color-surface-warm)', text: 'var(--color-text-muted)', label: 'Terminated' },
  assigned: { bg: 'var(--color-info-bg)', text: 'var(--color-primary)', label: 'Assigned' },
  ...
  declined: { bg: 'var(--color-danger-bg)', text: 'var(--color-danger-text)', label: 'Declined' },
};

function StatusBadge({ status }) {
  const s = STATUS_COLORS[status] || { bg: 'var(--color-surface-warm)', text: 'var(--color-text-muted)', label: status };
  return (
...
const INSURANCE_COLORS = {
  current: { bg: 'var(--color-success-bg)', text: 'var(--color-success-text)', label: 'Current' },
  expiring_soon: { bg: 'var(--color-warning-bg)', text: 'var(--color-warning-text)', label: 'Expiring Soon' },
  expired: { bg: 'var(--color-danger-bg)', text: 'var(--color-danger-text)', label: 'Expired' },
  unverified: { bg: 'var(--color-surface-warm)', text: 'var(--color-text-muted)', label: 'Unverified' },
};
...
const RISK_COLORS = {
  ...
  high: { bg: 'var(--color-danger-bg)', text: 'var(--color-danger-text)' },
};
...
function ContractorListTab({ contractors, loading, onSelect, statusFilter, setStatusFilter }) {
  if (loading) return <div style={{ padding: 40, textAlign: 'center', color: 'var(--color-text-muted)' }}>Loading contractors...</div>;
  if (!contractors.length) return <div style={{ padding: 40, textAlign: 'center', color: 'var(--color-text-muted)' }}>No contractors found.</div>;
        ...
        {['all', 'active', 'pending', 'suspended'].map(f => (
          <button key={f} onClick={() => setStatusFilter(f)}
            style={{
              ...
              background: statusFilter === f ? '#446145' : '#fff', color: statusFilter === f ? '#fff' : 'var(--color-text)',
      ...
      <div style={{ display: 'grid', gap: 12 }}>
        {contractors.map(c => (
          <div key={c.id} onClick={() => onSelect(c)}
            style={{
              background: '#fff', border: '1px solid var(--color-border)', borderRadius: 12, padding: 16, cursor: 'pointer',
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 6, fontSize: 12, color: 'var(--color-text-muted)' }}>
                {c.is_verified && <span style={{ color: 'var(--color-success-text)' }}>Verified</span>}
...
function ContractorDetailView({ contractor, onBack, onRefresh }) {
  ...
  if (!detail) return <div style={{ padding: 40, textAlign: 'center', color: 'var(--color-danger-text)' }}>Failed to load contractor.</div>;
  ...
  return (
    <div>
      ...
      {/* Header */}
      <div style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 12, padding: 20, marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 20, color: 'var(--color-text)' }}>{detail.business_name}</h2>
            <div style={{ color: 'var(--color-text-muted)', fontSize: 14, marginTop: 4 }}>{detail.contact_person} - {detail.contractor_type}</div>
            <div style={{ color: 'var(--color-text-muted)', fontSize: 13, marginTop: 2 }}>{detail.email} | {detail.phone}</div>
          </div>
        ...
        {detail.relationship && (
          <div style={{ display: 'flex', gap: 8, marginTop: 16, borderTop: '1px solid var(--color-surface-warm)', paddingTop: 12 }}>
            ...
            {detail.relationship.status === 'active' && (
              <button onClick={() => handleRelationshipAction('suspended')}
                style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid var(--color-border)', background: '#fff', color: 'var(--color-warning-text)', cursor: 'pointer', fontSize: 13 }}>
                Suspend
              </button>
            )}
            ...
            {['active', 'suspended', 'pending'].includes(detail.relationship.status) && (
              <button onClick={() => handleRelationshipAction('terminated')}
                style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid var(--color-danger-bg)', background: '#fff', color: 'var(--color-danger-text)', cursor: 'pointer', fontSize: 13 }}>
                Terminate
              </button>
            )}
            {!detail.is_verified && (
              <button onClick={handleVerifyInsurance}
                style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid var(--color-border)', background: '#fff', color: 'var(--color-primary)', cursor: 'pointer', fontSize: 13 }}>
                Verify Insurance
              </button>
            )}
          </div>
        ...
        {tabs.map(t => (
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            style={{
              ...
              color: activeTab === t.key ? '#446145' : 'var(--color-text-muted)',
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
      <div style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 12, padding: 16 }}>
        <h3 style={{ margin: '0 0 12px', fontSize: 15, color: 'var(--color-text)' }}>Business Details</h3>
      ...
      <div style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 12, padding: 16 }}>
        <h3 style={{ margin: '0 0 12px', fontSize: 15, color: 'var(--color-text)' }}>Insurance & Verification</h3>
      ...
      <div style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 12, padding: 16 }}>
        <h3 style={{ margin: '0 0 12px', fontSize: 15, color: 'var(--color-text)' }}>Biosecurity</h3>
      ...
      <div style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 12, padding: 16 }}>
        <h3 style={{ margin: '0 0 12px', fontSize: 15, color: 'var(--color-text)' }}>Relationship & Performance</h3>
...
function InfoRow({ label, value }) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', borderBottom: '1px solid var(--color-surface-warm)', fontSize: 13 }}>
      <span style={{ color: 'var(--color-text-muted)' }}>{label}</span>
      <span style={{ color: 'var(--color-text)', fontWeight: 500 }}>{String(value)}</span>
    </div>
  );
}

function AssignmentsTab({ assignments }) {
  if (!assignments.length) return <div style={{ padding: 32, textAlign: 'center', color: 'var(--color-text-muted)' }}>No assignments found.</div>;
  return (
    <div style={{ display: 'grid', gap: 10 }}>
      {assignments.map(a => (
        <div key={a.id} style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 10, padding: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <span style={{ fontWeight: 500, fontSize: 14 }}>Task #{a.task_id}</span>
              {a.work_description && <span style={{ color: 'var(--color-text-muted)', fontSize: 13, marginLeft: 8 }}>{a.work_description}</span>}
            </div>
...
function MovementsTab({ movements }) {
  if (!movements.length) return <div style={{ padding: 32, textAlign: 'center', color: 'var(--color-text-muted)' }}>No movement records.</div>;
  return (
    <div style={{ display: 'grid', gap: 10 }}>
      {movements.map(m => (
        <div key={m.id} style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 10, padding: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <span style={{ fontWeight: 500, fontSize: 14 }}>{m.purpose || 'Site visit'}</span>
              {m.vehicle_registration && <span style={{ color: 'var(--color-text-muted)', fontSize: 12, marginLeft: 8 }}>{m.vehicle_registration}</span>}
            </div>
...
function TrainingTab({ training }) {
  if (!training.length) return <div style={{ padding: 32, textAlign: 'center', color: 'var(--color-text-muted)' }}>No training records.</div>;
  return (
    <div style={{ display: 'grid', gap: 10 }}>
      {training.map(t => (
        <div key={t.id} style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 10, padding: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <span style={{ fontWeight: 500, fontSize: 14 }}>Module #{t.training_module_id}</span>
              {t.priority && <span style={{ color: 'var(--color-text-muted)', fontSize: 13, marginLeft: 8 }}>({t.priority})</span>}
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
...
function MovementsListTab({ loading }) {
  ...
  if (movementsLoading) return <div style={{ padding: 40, textAlign: 'center', color: 'var(--color-text-muted)' }}>Loading movements...</div>;
  if (!movements.length) return <div style={{ padding: 40, textAlign: 'center', color: 'var(--color-text-muted)' }}>No movement records.</div>;
        ...
        <div key={m.id} style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 10, padding: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              {m.purpose && <span style={{ color: 'var(--color-text-muted)', fontSize: 13, marginLeft: 8 }}>- {m.purpose}</span>}
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <RiskBadge level={m.biosecurity_risk_level} />
              <StatusBadge status={m.status} />
  ...
  if (error) {
    return (
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: 24 }}>
        <div style={{ background: 'var(--color-danger-bg)', border: '1px solid #fca5a5', borderRadius: 12, padding: 24, textAlign: 'center' }}>
          <AlertTriangle size={32} color="#991b1b" style={{ marginBottom: 8 }} />
          <p style={{ color: 'var(--color-danger-text)', fontWeight: 500 }}>{error}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 22, color: 'var(--color-text)' }}>Contractor Management</h1>
          <p style={{ margin: '4px 0 0', color: 'var(--color-text-muted)', fontSize: 14 }}>Manage contractor relationships, assignments, and biosecurity compliance</p>
        </div>
        <button onClick={fetchContractors}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 8, border: '1px solid var(--color-border)', background: '#fff', cursor: 'pointer', fontSize: 13 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        ...
        <SummaryCard icon={<CheckCircle size={20} />} label="Active" value={contractors.filter(c => c.relationship?.status === 'active').length} color="var(--color-success-text)" />
        <SummaryCard icon={<Clock size={20} />} label="Pending" value={contractors.filter(c => c.relationship?.status === 'pending').length} color="var(--color-warning-text)" />
        <SummaryCard icon={<Shield size={20} />} label="Verified" value={contractors.filter(c => c.is_verified).length} color="var(--color-primary)" />
      </div>
          ...
          {[{ key: 'contractors', label: 'Contractors' }, { key: 'movements', label: 'Site Movements' }].map(t => (
            <button key={t.key} onClick={() => setPageTab(t.key)}
              style={{
                ...
                color: pageTab === t.key ? '#446145' : 'var(--color-text-muted)',
...
function SummaryCard({ icon, label, value, color }) {
  return (
    <div style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 12, padding: 16, textAlign: 'center' }}>
