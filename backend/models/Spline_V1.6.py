# -*- coding: utf-8 -*-
"""
Created on Thu Mar 14 19:59:10 2024
...
test = True #set to no if not test

input_variable = 'TEMP_DAILY_Tmean(C)_SPLINE_INPUTS' #naming convention identical when in test mode also

def create_directories(output_folder, image_folder, station_output_folder, exceptions_folder, testing_folder):
    for folder in [output_folder, image_folder, station_output_folder, exceptions_folder, testing_folder]:
...
if test:
    # For test mode
        ...
        exceptions_log_path = 'Z:\\Data\\REGEN SPLINE V1.4\\TEST\\Logs\\SPLINE_Exception_Logs'
else:
    # For production mode
        ...
        image_folder = f'Z:\\Data\\REGEN SPLINE V1.4\\OUTPUT DATA\\{input_variable.replace("_INPUTS", "_IMAGES")}'
        ...
        exceptions_log_path = 'Z:\\Data\\REGEN SPLINE V1.4\\Logs\\SPLINE_Exception_Logs'

# Create directories
create_directories(output_folder, image_folder, station_output_folder, exceptions_folder, testing_folder)

match = re.match(r'([^_]+_DAILY)', input_variable)
if match:
    station_type = match.group(1)
else:
    station_type = ''

lat_lon_height_file = f'Z:\\Data\\Climate_Station_Data\\New_Zealand\\STATION_INFORMATION_CLIFLO\\CLIFLO_RAW_{station_type}.csv'
Station_Network_file = 'Z:\\Data\\VCDN_5km\\VCDN_5km.csv'
lapse_rate = 0.6
lat_lon_height_df = pd.read_csv(lat_lon_height_file)
statistics_df = pd.DataFrame(columns=['Date', 'Mean_MSE', 'Std_MSE', 'RMSE', 'M_Stations', 'T_Stations', 'T_RMSE'])
...
class InterpolationPlotter:
    ...
    def plot_and_save(self):
        ...
        plt.legend()
        plt.tight_layout()
        plot_file = os.path.join(self.image_folder, f'{self.climate_variable}_interpolation_plot_{self.dates}.png')
...
for file_name in os.listdir(folder_path):
    if file_name.endswith('.csv'):
        ...
        if "Temp" in climate_variable:
                merged_df[f"Adj_{climate_variable}"] = (merged_df[climate_variable] + merged_df["Height"]/100 * lapse_rate)
        else:
                merged_df[f"Adj_{climate_variable}"] = merged_df[climate_variable]
        ...
        y = merged_df[f'Adj_{climate_variable}'].values.astype(float)
        ...
        if "(C)" in climate_variable:
                station_network[f"Adjusted_{climate_variable}"] = (
                    station_network[f"Interpolated_{climate_variable}"] - station_network["Elevation"]/100 * lapse_rate
                )
                testing_df[f"Adjusted_{climate_variable}"] = (
                    testing_df[f"Interpolated_{climate_variable}"] - testing_df["Height"]/100 * lapse_rate
                )
        
        measured = testing_df[f'{climate_variable}']
        predicted = testing_df[f'Adjusted_{climate_variable}']
        ...
        testing_df.to_csv(f'Z:\\Data\\REGEN SPLINE V1.4\\TEST\\TEMP_DAILY_Tmean(C)_SPLINE_EXCEPTIONS\\{date_str}.csv')
        if rmse > 0.4:
            ...
            for threshold_km in range(2, 11, 2):  # Iterate from 2 km to 10 km in steps of 2 km
                ...
                print(f'Stations: keep {len(stations_to_keep)} / remove {len(stations_to_remove)}')
                print(f"Threshold {threshold_km} km: Stations Removed: {len(stations_to_remove)}, Stations Remaining: {len(stations_to_keep)}")
                ...
                merged_df = merged_df[~merged_df["Station"].isin(stations_to_remove)]
                print(f'DF: testing {len(testing_df)} / merged {len(merged_df)}')
                X = merged_df[['Longitude', 'Latitude']].values.astype(float)
                y = merged_df[f'Adj_{climate_variable}'].values.astype(float)
                valid_indices = np.logical_and(~np.isnan(X).any(axis=1), ~np.isnan(y))
                ...
                if "(C)" in climate_variable:
                    merged_df[f"Interpolated_{climate_variable}"] = interpolated_temp - merged_df["Height"]/100 * lapse_rate
                else:
                    merged_df[f"Interpolated_{climate_variable}"] = interpolated_temp
    ...
    plotter = InterpolationPlotter(
            ...
            interpolated_values=station_network[f"Adjusted_{climate_variable}"],
            merged_df=merged_df,
...
stats_loc = os.path.join(log_path,f'{station_type}_Cross_Validation_Summary.csv')
