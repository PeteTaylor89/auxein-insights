...
input_variable = 'PRECIPITATION_DAILY_Amount(mm)_SPLINE_INPUTS' #naming convention identical when in test mode also
...
problem_stations = [3595, 10141, 37002]
...
for file_name in os.listdir(folder_path):
    if file_name.endswith('.csv'):
        ...
        station_network[f"Interpolated_{climate_variable}"] = np.round(
            rbf_interpolator.predict(station_network[['Longitude', 'Latitude']].values), 2
        )
        
        testing_df[f"Interpolated_{climate_variable}"] = np.round(
            rbf_interpolator.predict(testing_df[['Longitude', 'Latitude']].values), 2
        )
