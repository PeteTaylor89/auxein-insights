"""
One-time script to create TDC (Tasman District Council) station records in database

Usage:
    python setup_tdc_stations.py --dry-run   # Preview what would be inserted
    python setup_tdc_stations.py             # Actually insert records
"""
...
from ingestion.config.tdc_sites import TDC_SITES


def setup_tdc_stations(dry_run: bool = False):
    """Create TDC weather station records"""
    ...
    with Session() as session:
        ...
        for station_code, config in TDC_SITES.items():
            try:
                ...
                result = session.execute(text("""
                    ...
                    WHERE station_code = :code AND data_source = 'TDC'
                """), {'code': station_code})
                ...
                if dry_run:
                    ...
                    session.execute(text("""
                        ...
                        VALUES
                            (:code, :name, 'TDC', :site_name, :lat, :lon, :elevation,
        ...
        print(f"  Total:   {len(TDC_SITES)}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Setup TDC weather stations in database')
    parser.add_argument('--dry-run', action='store_true',
                        help='Preview changes without writing to database')
    args = parser.parse_args()

    setup_tdc_stations(dry_run=args.dry_run)
