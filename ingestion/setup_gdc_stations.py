"""
One-time script to create GDC (Gisborne District Council) station records in database

Usage:
    python setup_gdc_stations.py --dry-run   # Preview what would be inserted
    python setup_gdc_stations.py             # Actually insert records
"""
...
from ingestion.config.gdc_sites import GDC_SITES


def setup_gdc_stations(dry_run: bool = False):
    """Create GDC weather station records"""
    ...
    with Session() as session:
        ...
        for station_code, config in GDC_SITES.items():
            try:
                ...
                result = session.execute(text("""
                    ...
                    WHERE station_code = :code AND data_source = 'GDC'
                """), {'code': station_code})
                ...
                if dry_run:
                    ...
                    session.execute(text("""
                        ...
                        VALUES
                            (:code, :name, 'GDC', :site_name, :lat, :lon, :elevation,
        ...
        print(f"  Total:   {len(GDC_SITES)}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Setup GDC weather stations in database')
    parser.add_argument('--dry-run', action='store_true',
                        help='Preview changes without writing to database')
    args = parser.parse_args()

    setup_gdc_stations(dry_run=args.dry_run)
