"""
TDC (Tasman District Council) weather data ingestion
API: Hilltop Server at http://envdata.tasman.govt.nz/data.hts

Key differences from MDC/GW:
  - HTTP (not HTTPS)
  - Rainfall requires Method=Total parameter for hourly totals
  - Temperature measurement is 'Air Temperature (continuous)' not 'Air Temperature'
  - Humidity measurement is 'Relative humidity' not 'Humidity'
  - Backfill must be done site-by-site per TDC agreement (avoid load)
"""
...
class TDCIngestion:
    ...
    def __init__(self):
        self.data_source = 'TDC'
        self.base_url = TDC_API_BASE
    ...
    def get_active_stations(self):
        """Get all active TDC stations from database"""
    ...
    def fetch_data(self, site_name: str, measurement: str,
        ...
        """Fetch data from TDC Hilltop API

        Args:
            site_name: Exact site name for API
            measurement: Measurement name (e.g., 'Air Temperature (continuous)')
    ...
    def parse_response(self, station_id: int, xml_text: str,
        ...
        for elem in root.iter('E'):
            try:
                t_elem = elem.find('T')
                # TDC may use I1 or Value for data values
    ...
    def run(self, period: str = 'incremental', backfill_days: int = None,
        ...
        """
        print(f"\n{'='*60}")
        print(f"Starting TDC ingestion at {datetime.now()}")
        ...
        if station_code:
            ...
            if not stations:
                print(f"⚠ Station '{station_code}' not found in active TDC stations")
        ...
        print(f"Found {len(stations)} active TDC station(s)\n")
        ...
        print(f"{'='*60}")
        print(f"TDC ingestion complete at {datetime.now()}")
        if dry_run:
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run TDC weather data ingestion')
    ...
    parser.add_argument('--station', type=str,
                        help='Station code to run a single station (e.g., TDC_RICHMOND_ROOF)')
