"""
HBRC (Hawke's Bay Regional Council) weather data ingestion
API: Hilltop Server at https://data.hbrc.govt.nz/Envirodata/EMAR.hts

Key differences from MDC:
  - No Collection parameter needed (MDC requires Collection=Climate|Rainfall)
  - Different measurement names (e.g., 'Average Air Temperature' vs 'Air Temperature')
  - Has Solar Radiation data at climate stations
  - Data element may use <I1> or <Value> depending on measurement type
"""
...
class HBRCIngestion:
    """Ingestion class for HBRC Hilltop weather data"""
    
    def __init__(self):
        self.data_source = 'HBRC'
        self.base_url = HBRC_API_BASE
    ...
    def get_active_stations(self):
        """Get all active HBRC stations from database"""
    ...
    def fetch_data(self, site_name: str, measurement: str, 
        ...
        """Fetch data from HBRC Hilltop API
        
        Args:
            site_name: Exact site name for API (e.g., 'Bridge Pa Climate')
            measurement: Measurement name (e.g., 'Average Air Temperature')
            start_time: Start of time range
            end_time: End of time range
            interval: Optional aggregation interval (e.g., '30 minutes', '1 hour')
        
        Note: Unlike MDC, HBRC does NOT require a Collection parameter.
        """
    ...
    def parse_response(self, station_id: int, xml_text: str, 
        ...
        <Hilltop>
          <Agency>HBRC</Agency>
          <Measurement SiteName="Bridge Pa Climate">
            <DataSource>
              <ItemInfo ItemNumber="0">
                <ItemName>Average Air Temperature</ItemName>
                <Units>deg C</Units>
              </ItemInfo>
            </DataSource>
            <Data>
              <E><T>2026-01-24T00:00:00</T><I1>18.5</I1></E>
              ...
            </Data>
          </Measurement>
        </Hilltop>
        ...
        for elem in root.iter('E'):
            try:
                t_elem = elem.find('T')
                # HBRC may use I1 or Value for data values
    ...
    def insert_data(self, records: list) -> int:
        ...
        with self.Session() as session:
            try:
                ...
                session.rollback()
                # A write that failed is not a station that succeeded. Without this the
                # caller prints a tick against 0 records and the process exits 0, so the
                # backfill driver logs OK — which is how the DST-duplicate abort silently
                # lost every full year of the 2026-08-19 re-backfill.
    ...
    def run(self, period: str = 'incremental', backfill_days: int = None,
        ...
        """
        print(f"\n{'='*60}")
        print(f"Starting HBRC ingestion at {datetime.now()}")
        ...
        if station_code:
            ...
            if not stations:
                print(f"⚠ Station '{station_code}' not found in active HBRC stations")
        ...
        print(f"Found {len(stations)} active HBRC station(s)\n")
        ...
        print(f"{'='*60}")
        print(f"HBRC ingestion complete at {datetime.now()}")
        if dry_run:
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run HBRC weather data ingestion')
