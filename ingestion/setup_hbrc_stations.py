"""
One-time script to create HBRC station records in database

Usage:
    python setup_hbrc_stations.py --dry-run   # Preview what would be inserted
    python setup_hbrc_stations.py             # Actually insert records
"""
...
from ingestion.config.hbrc_sites import HBRC_SITES


def setup_hbrc_stations(dry_run: bool = False):
    """Create HBRC weather station records"""
    ...
    with Session() as session:
        ...
        for station_code, config in HBRC_SITES.items():
            try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Setup HBRC weather stations in database')
    parser.add_argument('--dry-run', action='store_true', 
                        help='Preview changes without writing to database')
    args = parser.parse_args()
    
    setup_hbrc_stations(dry_run=args.dry_run)