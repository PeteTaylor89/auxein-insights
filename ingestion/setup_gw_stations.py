"""
One-time script to create GW (Greater Wellington) station records in database

Usage:
    python setup_gw_stations.py --dry-run   # Preview what would be inserted
    python setup_gw_stations.py             # Actually insert records
"""
...
from ingestion.config.gw_sites import GW_SITES


def setup_gw_stations(dry_run: bool = False):
    """Create GW weather station records"""
    ...
    with Session() as session:
        ...
        for station_code, config in GW_SITES.items():
            try:
                ...
                result = session.execute(text("""
                    ...
                    WHERE station_code = :code AND data_source = 'GW'
                """), {'code': station_code})
                ...
                notes_dict = {
                    ...
                    "data_from": config.get('data_from'),
                }
                ...
                if dry_run:
                    ...
                    print(f"    Elevation: {config.get('elevation')}")
                    print(f"    Measurements: {config['measurements']}")
                    print(f"    Data from: {config.get('data_from')}")
                    created += 1
                else:
                    ...
                    session.execute(text("""
                        ...
                        VALUES 
                            (:code, :name, 'GW', :site_name, :lat, :lon, :elevation,
        ...
        print(f"  Total:   {len(GW_SITES)}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Setup GW weather stations in database')
    parser.add_argument('--dry-run', action='store_true', 
                        help='Preview changes without writing to database')
    args = parser.parse_args()
    
    setup_gw_stations(dry_run=args.dry_run)