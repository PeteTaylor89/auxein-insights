...
| **5** | **NARROWED 2026-09-16 — `packages/insights` ONLY.** Strip `/admin` from Insights; bare permanent redirects (§4.1); drop the Insights header links; delete `useAdminAuth.js`. **Grow (`packages/web`) KEEPS its `/admin`** — useful in place, so platform admin deliberately lives in two apps and the Grow trees must be kept in step by hand | 4 + Phase 6 live | code |
| **6** | ✅ **DONE 2026-09-16.** `admin.auxein.co.nz` live: cert, bucket `auxein-admin-web`, CF `EFRES9N55UALS`, OAC, Route53 A+AAAA, first deploy. IP allow-list still outstanding. **Infra (Pete):** ACM cert us-east-1, `auxein-admin-web` bucket, CloudFront + OAC + SPA error routing, Route53 alias, IP allow-list, first deploy. Then redeploy the API for the Phase 0 CORS change | 0 | Pete |
